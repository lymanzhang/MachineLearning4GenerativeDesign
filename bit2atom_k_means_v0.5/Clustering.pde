void clustergroup(clusterPoint[] cpk) {
...
void updateClusterGroup() {
  ...
  for (int i = 0; i < spk.length; i ++) {
    ...
    spk[i] = new clusterPoint(centerPoint, i, 0, 1);
...
void saveClusterTable() {
  clusterTable = new Table();

  clusterTable.addColumn("SampleID");
  clusterTable.addColumn("Class");
  
  for (int i = 0; i < spk.length; i ++) {
    for (int j = 0; j < sp.length; j ++) {
      if (sp[j].clusterGroup == i) {
        TableRow newRow = clusterTable.addRow();
        newRow.setInt("SampleID", j);
        newRow.setInt("Class", i);
      }
    }
  }
  saveTable(clusterTable, "data/clusterTable.csv");
}
